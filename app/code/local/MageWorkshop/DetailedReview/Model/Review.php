...
class MageWorkshop_DetailedReview_Model_Review extends Mage_Review_Model_Review
{
    ...
    public function sendEmail($mailersData)
    {
        ...
        if(is_array($recipient = $mailersData->getRecipientEmail())) {
            foreach($recipient as $k => $email) {
                $emailInfo->addTo($email, (is_array($name = $mailersData->getRecipientName())? $name[$k]:$mailersData->getRecipientName()));
            }
        } else {
            $emailInfo->addTo($recipient, $mailersData->getRecipientName());
        }
    ...
    {
        $numberOfReviewDisplay = Mage::getStoreConfig('detailedreview/list_options/qty_default');
        if (is_null($this->_reviewsCollection) || $processForce) {
            ...
            if ($product->isGrouped()) {
                ...
                $reviewsCollection->addOwnershipInfo($product->getId(), $childrenIds);
            } else {
            ...
            $this->_reviewsCollection = $reviewsCollection->setCurPage($params['r_p'])->setPageSize($numberOfReviewDisplay);
    ...
    {
        $result = false;
        /** @var Mage_Rating_Model_Rating_Option_Vote $votes */
        $votes = $this->getRatingVotes();

        if ($votes && $votes->getItems()) {
            $result = true;
        }
