...
class MageWorkshop_DetailedReview_Model_Uninstall extends Mage_Core_Model_Abstract
{
    ...
    public function clearDatabaseInformation()
    {
        ...
        $verifiedBuyerIndexer = $indexer->getProcessByCode(MageWorkshop_DetailedReview_Helper_Config::DETAILEDREVIEW_VERIFIED_BUYER_INDEXER_CODE);
        if($verifiedBuyerIndexer) {
            $verifiedBuyerIndexer->delete();
        }

        $productAttrIndexer = $indexer->getProcessByCode(MageWorkshop_DetailedReview_Helper_Config::DETAILEDREVIEW_PRODUCT_ATTR_INDEXER_CODE);
        if($productAttrIndexer) {
            $productAttrIndexer->delete();
        }
        ...
        $complaintTypeTable = $coreResource->getTableName('detailedreview/complaint_type');
        $reviewCustomerComplaintTable = $coreResource->getTableName('detailedreview/review_customer_complaint');
        ...
        $sql .= "DROP TABLE IF EXISTS `$complaintTypeTable`;";
        $sql .= "DROP TABLE IF EXISTS `$reviewCustomerComplaintTable`;";
