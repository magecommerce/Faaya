<?php
class Faaya_Orderapi_Model_Orderapi extends Mage_Core_Model_Abstract
{
    protected function _construct()
    {
		parent::_construct();
        $this->_init('orderapi/orderapi');
    }
    public function exportOrders(){
        $orderCollection = Mage::getModel('sales/order')->getCollection()->addAttributeToFilter('ref_id', '');
            ...
            try{
                ...
                //exit;
                 ...
                 foreach($itemArray as $items){ 
                    ...
                    json_decode($json);
                    //echo  'POST JSON: '. $json; 
                    Mage::log($json, null,'orderAPI-Cron.log');
                    $ch = curl_init("http://52.7.153.114/SeaUtilityTest/SEAUtilityService.svc/OrderImportFaaya");
                    ...
                    Mage::log($output, null,'orderAPI-Cron.log');
                }
            }
            catch (Mage_Core_Exception $e) {
                Mage::log($e->getMessage(), null, $orderId.'-update-order-cron-error.log');
            }
        }
    }
}
