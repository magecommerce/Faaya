...
//        }.bind(this));
    ...
    openSection: function(section) {
        ...
        if(section.id != this.currentSection) {
            ...
            var lbl = jQuery("."+cur+" .step-title span").text();
            jQuery(".page-title h1").text(lbl);
            $(this.currentSection).addClassName('active');
            $$("."+this.currentSection).first().addClassName('active');
            if(this.currentSection == "opc-payment"){ 
                setTimeout(function(){ jQuery(".payment-btn").removeAttr("disabled");}, 500);
                
               //setTimeout(function(){ jQuery("#p_method_cryozonic_stripe").prop("checked", true);} , 1000);
            }
